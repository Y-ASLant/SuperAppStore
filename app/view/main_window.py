...
class MainWindow(MSFluentWindow):
    def __init__(self):
        ...
        self.githubInterface = GitHubInterface(self)
    ...
    def connectSignalToSlot(self):
        ...
        self.downloadInterface.signals.moveToCompletedSignal.connect(
    ...
    def onDownloadComplete(self, app_id):
        ...
        # 从正在下载的临时集合中移除
        if (
            hasattr(self.applicationInterface, "tracking_downloads")
            and app_id in self.applicationInterface.tracking_downloads
        ):
    ...
    def fetchAppsList(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"应用列表已更新: {APPS_FILE}")
            else:
                logger.error(f"获取应用列表失败，状态码: {response.status_code}")

        except Exception as e:
            logger.exception(f"获取应用列表出错: {str(e)}")
    ...
    def initNavigation(self):
        ...
        self.githubInterface.setObjectName("githubInterface")
        ...
        self.addSubInterface(self.githubInterface, FIF.GITHUB, self.tr("GitHub"))
