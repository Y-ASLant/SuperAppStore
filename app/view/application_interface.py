...
class ApplicationInterface(ScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        self.gameSortComboBox.currentIndexChanged.connect(
            self.__onGameSortOptionChanged
        )
        ...
        self.addSubInterface(
            self.appPage, "appPage", self.tr("  应用  "), FIF.APPLICATION
        )
            ...
            if not os.path.exists(APPS_FILE):
                ...
                try:
                    ...
                    with open(APPS_FILE, "w", encoding="utf-8") as f:
                        json.dump([], f)
                    self.__showSuccessNotification(
                        "已创建空的应用列表文件，请刷新或重启应用"
                    )
                    # 使用空列表继续
                    apps = []
    ...
    def __updateAppList(self):
        ...
        if not self.filtered_apps:
            ...
            for app in self.filtered_apps:
                ...
                if (
                    app_id in self.downloaded_app_ids
                    or app_id in self.tracking_downloads
                ):
    ...
    def __updateGameList(self):
        ...
        if not self.filtered_games:
            ...
            for game in self.filtered_games:
                ...
                if (
                    game_id in self.downloaded_app_ids
                    or game_id in self.tracking_downloads
                ):
                    card.downloadButton.setVisible(False)
    ...
    def __onAppSearchTextChanged(self, text):
        ...
        if not text:
            ...
            self.filtered_apps = [
                ...
                or (
                    app.get("description")
                    and text.lower() in app["description"].lower()
                )
            ]
        ...
        if not text:
            ...
            self.filtered_games = [
                ...
                or (
                    ...
                    and text.lower() in game["description"].lower()
                )
            ]
    ...
    def __onAppSortOptionChanged(self, index):
        ...
        if index == 0:  # 默认排序
            if self.filtered_apps:
                original_order_dict = {
                    app["name"]: i for i, app in enumerate(self.original_apps_order)
                }
                self.filtered_apps.sort(
                    key=lambda x: original_order_dict.get(x["name"], 999999)
                )
        elif index == 1:  # 名称排序
    ...
    def __onGameSortOptionChanged(self, index):
        ...
        if index == 0:  # 默认排序
            if self.filtered_games:
                original_order_dict = {
                    game["name"]: i for i, game in enumerate(self.original_games_order)
                }
                self.filtered_games.sort(
                    key=lambda x: original_order_dict.get(x["name"], 999999)
                )
        elif index == 1:  # 名称排序
    ...
    def __onDownloadApp(self, app_data):
        ...
        try:
            if app_data.get("download_url"):
                ...
                self.__showSuccessNotification(
                    f"已添加 {app_data['name']} {app_data.get('version', '')} 到下载队列"
                )
            else:
                self.__showErrorNotification(
                    f"应用 {app_data['name']} 没有可用的下载链接"
                )
        except Exception as e:
