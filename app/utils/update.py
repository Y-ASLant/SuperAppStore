...
class CheckUpdateThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if github_token:
                # GitHub Personal Access Token 使用 'token' 前缀
            ...
            # 清理和格式化更新日志
            # 移除 HTML 标签（包括 img 标签）
            changelog_body = re.sub(r"<[^>]+>", "", changelog_body)
            # 移除 Markdown 图片语法 ![alt](url)
            changelog_body = re.sub(r"!\[([^\]]*)\]\([^\)]+\)", "", changelog_body)
            # 移除多余的空行
            changelog_body = re.sub(r"\n{3,}", "\n\n", changelog_body)

            # 将 body 按行分割，过滤空行和只有符号的行
            changelog_lines = []
            ...
            changelog_str = (
                "\n".join(changelog_lines) if changelog_lines else "无更新说明"
            )
            ...
            if e.response.status_code == 404:
                # 404 表示还没有发布任何 release，这是正常情况
                print("尚未发布任何版本")
                self.updateCheckFinished.emit(False, "", "", "", False)
            else:
                error_msg = f"检查更新失败: {e.response.status_code}"
                print(f"检查更新失败: {e}")
...
class UpdateChecker(QObject):
    ...
    def _on_thread_finished(
        self, has_update, remote_version, changelog_str, download_url, force_update
    ):
        """处理线程完成信号"""
        # 转发信号
        self.updateCheckFinished.emit(
...
class UpdateManager(QObject):
    ...
    def on_update_check_finished(
        ...
        if has_update:
            ...
            # 没有更新或检查失败
            if changelog and (
                changelog.startswith("网络") or changelog.startswith("检查更新失败")
            ):
                # 只在检查失败时显示错误信息
                Notification.error(
                    title="检查更新失败",
    ...
    def _open_download_url(self, url):
        ...
        try:
            ...
            Notification.error(
                title="下载失败", content=error_msg, duration=3000, parent=self.parent()
            )
