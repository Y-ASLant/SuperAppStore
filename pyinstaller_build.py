    ...
    try:
        ...
        if user_input == "" or user_input == "y" or user_input == "yes":
            ...
            if python_version >= (3, 8):
                ...
                log_warning(f"  Python {platform.python_version()} (建议使用3.8+)")
            ...
            if not found_manager and shutil.which("pip"):
                ...
                log_warning("  未找到包管理器 (pip/uv/poetry等)")
        ...
        for pkg_type in package_types:
            if pkg_type == "deb":
                if check_tool_installed("dpkg-deb", silent=True):
                    ...
                    log_warning("  dpkg-deb 未安装，DEB包生成可能失败")
            elif pkg_type == "rpm":
                if check_tool_installed("rpmbuild", silent=True):
                    log_success(" RPM包支持已就绪")
                else:
                    log_warning("  rpmbuild 未安装，RPM包生成可能失败")
...
def copy_additional_files():
    ...
    if not build_output_dir.exists():
        log_warning(f"  构建输出目录不存在: {build_output_dir}")
        return
    ...
    for dir_name in copy_dirs:
        ...
        if src_dir.exists() and src_dir.is_dir():
            ...
            log_warning(f"  目录不存在，跳过: {src_dir}")
...
"""
    ...
    args = [
        ...
        "App.ico;.",
        "--add-data",
        f"{qt_plugins_path};PyQt5/Qt5/plugins",
        "--runtime-hook",
        str(hook_file),
        "--collect-all",
        "qfluentwidgets",
